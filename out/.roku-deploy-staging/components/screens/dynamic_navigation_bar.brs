...
sub handleNavigationResponse()
    ...
    if responseData <> invalid and responseData <> ""
        ...
        if parsedData <> invalid and parsedData.data <> invalid
            ...
            ' Inject TV Guide and User Channels tabs (shown for all users)
            ...
            print "DynamicNavigationBar.brs - [handleNavigationResponse] *** CALLING injectUserChannelsTab ***"
            navData = injectUserChannelsTab(navData)
            print "DynamicNavigationBar.brs - [handleNavigationResponse] *** RETURNED FROM injectUserChannelsTab ***"
            print "DynamicNavigationBar.brs - [handleNavigationResponse] navData count after injections: " + navData.Count().ToStr()
...
function injectUserChannelsTab(navData as object) as object
    print "DynamicNavigationBar.brs - [injectUserChannelsTab] ========================================"
    print "DynamicNavigationBar.brs - [injectUserChannelsTab] Starting User Channels injection"
    print "DynamicNavigationBar.brs - [injectUserChannelsTab] Input navData count: " + navData.Count().ToStr()
    print "DynamicNavigationBar.brs - [injectUserChannelsTab] ========================================"
    
    ' Log all input nav items first
    print "DynamicNavigationBar.brs - [injectUserChannelsTab] Input navigation items:"
    for i = 0 to navData.Count() - 1
        if navData[i] <> invalid
            itemTitle = ""
            itemId = ""
            if navData[i].title <> invalid then itemTitle = navData[i].title
            if navData[i].id <> invalid then itemId = navData[i].id.ToStr()
            print "DynamicNavigationBar.brs - [injectUserChannelsTab]   [" + i.ToStr() + "] id=" + itemId + ", title=" + itemTitle
        end if
    end for
    
    ' Check if User Channels (id=14) already exists in the navigation
    userChannelsExists = false
    for i = 0 to navData.Count() - 1
        if navData[i] <> invalid and navData[i].id <> invalid
            itemId = navData[i].id
            if Type(itemId) = "roString" or Type(itemId) = "String"
            ...
            if itemId = 14
                userChannelsExists = true
                print "DynamicNavigationBar.brs - [injectUserChannelsTab] User Channels already exists at index " + i.ToStr()
                exit for
            end if
        end if
    end for
    
    ' If User Channels already exists, don't inject it again
    if userChannelsExists
        print "DynamicNavigationBar.brs - [injectUserChannelsTab] User Channels already in navigation, skipping injection"
        return navData
    end if
    
    ' User Channels is shown for all users (authenticated and unauthenticated)
    print "DynamicNavigationBar.brs - [injectUserChannelsTab] User Channels will be shown for all users"
    print "DynamicNavigationBar.brs - [injectUserChannelsTab] Proceeding to inject User Channels tab"
    
    ' Find the index of "Series/TV Shows" (id: 2) to insert User Channels after it
    insertIndex = -1
    for i = 0 to navData.Count() - 1
        if navData[i] <> invalid and navData[i].id <> invalid
            itemId = navData[i].id
            ...
            print "DynamicNavigationBar.brs - [injectUserChannelsTab] Checking item " + i.ToStr() + ": id=" + itemId.ToStr() + " (original type: " + originalType + ")"
            if itemId = 2 ' Series/TV Shows
                insertIndex = i + 1
                print "DynamicNavigationBar.brs - [injectUserChannelsTab] *** FOUND Series at index " + i.ToStr() + " ***"
                print "DynamicNavigationBar.brs - [injectUserChannelsTab] Will insert User Channels at index " + insertIndex.ToStr()
                exit for
            end if
        end if
    end for
    
    ' If Series not found, insert at the end (before Profile which is added separately)
    if insertIndex = -1
        print "DynamicNavigationBar.brs - [injectUserChannelsTab] Series not found, inserting User Channels at end"
        insertIndex = navData.Count()
    end if
    
    ' Create User Channels nav item
    userChannelsItem = {
        id: 14,
        title: "User Channels",
        type: "user_channels",
    ...
    print "DynamicNavigationBar.brs - [injectUserChannelsTab] Created User Channels item: id=14, title=User Channels"
    
    ' Insert User Channels at the determined position
    print "DynamicNavigationBar.brs - [injectUserChannelsTab] Building new navigation array with User Channels at index " + insertIndex.ToStr()
    newNavData = []
    for i = 0 to navData.Count() - 1
        if i = insertIndex
            newNavData.Push(userChannelsItem)
            print "DynamicNavigationBar.brs - [injectUserChannelsTab] >>> INSERTED User Channels tab at position " + newNavData.Count().ToStr()
        end if
        newNavData.Push(navData[i])
    end for
    ...
    if insertIndex = navData.Count()
        newNavData.Push(userChannelsItem)
        print "DynamicNavigationBar.brs - [injectUserChannelsTab] >>> INSERTED User Channels tab at END, position " + newNavData.Count().ToStr()
    end if
    
    print "DynamicNavigationBar.brs - [injectUserChannelsTab] ========================================"
    print "DynamicNavigationBar.brs - [injectUserChannelsTab] FINAL navigation items count: " + newNavData.Count().ToStr()
    print "DynamicNavigationBar.brs - [injectUserChannelsTab] ========================================"
    ...
    for i = 0 to newNavData.Count() - 1
        if newNavData[i] <> invalid
            ...
            if newNavData[i].title <> invalid then itemTitle = newNavData[i].title
            if newNavData[i].id <> invalid then itemId = newNavData[i].id.ToStr()
            print "DynamicNavigationBar.brs - [injectUserChannelsTab] FINAL [" + i.ToStr() + "]: id=" + itemId + ", title=" + itemTitle
        end if
    end for
...
function filterNavByAuth(navData as object) as object
    ...
    ' Not authenticated - filter out auth-only tabs (Age Restricted=15, Personal=16)
    ' User Channels (14) is now available for all users
    filtered = []
...
function buildNavigationItems() as boolean
    ...
    parentScene = m.top.getParent()
    if parentScene <> invalid
        print "DynamicNavigationBar.brs - [buildNavigationItems] Triggering content screens rebuild with " + navigationData.Count().ToStr() + " items"
...
function handleNavigationKey(key as string, press as boolean) as boolean
    ...
    if key = "up" then
        ...
        ' Switch screen WITHOUT giving it focus (focus stays on navigation)
        parentScene = m.top.getParent()
        if parentScene <> invalid
            ...
            print "DynamicNavigationBar.brs - [handleNavigationKey] UP: Calling switchScreenContent(" + screenIndex.ToStr() + ") - focus stays on nav"
            if m.currentIndex < m.navItems.count() and m.navItems[m.currentIndex] <> invalid
                navLabel = m.navItems[m.currentIndex].getChild(1)
                if navLabel <> invalid
        ...
        if parentScene <> invalid
            ...
            print "DynamicNavigationBar.brs - [handleNavigationKey] DOWN: Calling switchScreenContent(" + screenIndex.ToStr() + ") - focus stays on nav"
            if m.currentIndex < m.navItems.count() and m.navItems[m.currentIndex] <> invalid
                navLabel = m.navItems[m.currentIndex].getChild(1)
                if navLabel <> invalid
            ...
            if screenIndex = accountIndex
                ...
                if dynamicScreensContainer <> invalid
                    ...
                    if contentScreen <> invalid
                        print "DynamicNavigationBar.brs - [handleNavigationKey] RIGHT: Found content screen at index " + screenIndex.ToStr()
                        print "DynamicNavigationBar.brs - [handleNavigationKey] RIGHT: Screen visible: " + contentScreen.visible.ToStr()
                        print "DynamicNavigationBar.brs - [handleNavigationKey] RIGHT: Screen ID: " + contentScreen.id
                        print "DynamicNavigationBar.brs - [handleNavigationKey] RIGHT: Current explicitContentFocusRequested: " + contentScreen.explicitContentFocusRequested.ToStr()
                        
                        ' Increment counter to trigger focus (using counter instead of boolean to avoid re-trigger issues)
                        contentScreen.explicitContentFocusRequested = contentScreen.explicitContentFocusRequested + 1
                        
                        print "DynamicNavigationBar.brs - [handleNavigationKey] RIGHT: New explicitContentFocusRequested: " + contentScreen.explicitContentFocusRequested.ToStr()
                    else
                        print "DynamicNavigationBar.brs - [handleNavigationKey] RIGHT: ERROR - Content screen not found at index " + screenIndex.ToStr()
                    end if
                else
                    print "DynamicNavigationBar.brs - [handleNavigationKey] RIGHT: ERROR - dynamicScreensContainer not found"
                end if
                
                ' Don't call selectAndScreenSpecificFocus - it causes double-trigger
                ' The explicitContentFocusRequested mechanism above handles focus
...
sub selectAndScreenSpecificFocus(index as integer)
    ...
    if parentScene <> invalid
        ...
        if screenIndex = accountIndex
            ...
            ' After setting screen focus, the explicitContentFocusRequested mechanism will handle
            ' focusing the appropriate child component (e.g., TimeGrid for TV Guide, RowList for others)
            ' NOTE: We used to call setFocus(true) here, but that caused double-focus issues
            ' where the screen container would get focus first, then the child component
            print "DynamicNavigationBar.brs - [selectAndScreenSpecificFocus] Content screen focus will be handled by explicitContentFocusRequested mechanism"
        end if
    else
...
function convertNavIndexToScreenIndex(navIndex as integer) as integer
    ...
    ' Navigation structure (TV Guide and User Channels shown for all users):
    '   0=Home, 1=Live TV, 2=TV Guide, 3=Movies, 4=Series, 5=User Channels, 6=Profile (unauthenticated)
    '   0=Home, 1=Live TV, 2=TV Guide, 3=Movies, 4=Series, 5=User Channels, 6=Age Restricted, 7=Personal, 8=Profile (authenticated)
