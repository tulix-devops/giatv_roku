function init()
    ...
    ' Loader timer for auto-hide after 3 seconds
    m.loaderTimer = invalid
...
sub onNavigationDataReceived()
    ...
    if navigationData <> invalid and navigationData.Count() > 0
        ...
        ' Timer will auto-hide loader after 3 seconds
    else
...
function checkAuthenticationForUserChannels() as boolean
    ...
    ' (Age Restricted, Personal)
    ' Note: User Channels (ID 14) is now available for all users
...
function filterNavigationByAuth(navigationData as object) as object
    ...
    ' Remove Age Restricted (ID 15) and Personal (ID 16) if user is not authenticated
    ' User Channels (ID 14) is now available for ALL users
    ...
    for each navItem in navigationData
        if navItem <> invalid and navItem.id <> invalid
            ...
            ' Age Restricted (ID 15), Personal (ID 16) - User Channels (ID 14) is now available for all users
            if itemId = 15 or itemId = 16
                if isAuthenticated
                    filteredItems.Push(navItem)
                end if
            else
                ' Keep all other items (Home, Live TV, TV Guide, Movies, Series, User Channels)
                filteredItems.Push(navItem)
            end if
        end if
    end for
    
    print "HomeScene.brs - [filterNavigationByAuth] Filtered: " + filteredItems.Count().ToStr() + " items (was " + navigationData.Count().ToStr() + ")"
...
' Global loader functions with 3-second auto-hide timer
sub showGlobalLoader(message = "Loading..." as string)
    print "HomeScene.brs - [showGlobalLoader] Showing loader: " + message
    ...
    if m.loaderTimer <> invalid
        m.loaderTimer.control = "stop"
        m.loaderTimer = invalid
    end if
    
    ' Create 3-second timer to auto-hide loader
    m.loaderTimer = CreateObject("roSGNode", "Timer")
    m.loaderTimer.duration = 3
    m.loaderTimer.repeat = false
    m.loaderTimer.observeField("fire", "onLoaderTimerFired")
    m.loaderTimer.control = "start"
    print "HomeScene.brs - [showGlobalLoader] Started 3-second auto-hide timer"
end sub

sub hideGlobalLoader()
    print "HomeScene.brs - [hideGlobalLoader] Hiding loader"
...
sub onLoaderTimerFired()
    print "HomeScene.brs - [onLoaderTimerFired] 3 seconds elapsed, auto-hiding loader"
    if m.globalLoader <> invalid
        m.globalLoader.visible = false
    end if
    stopLoadingAnimation()
