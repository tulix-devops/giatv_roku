...
sub GetHomeScreenData()
    ...
    if request.AsyncGetToString()
        print "HomeScreenAPI.brs - [GetHomeScreenData] Request sent successfully, waiting for response..."
        while true
            msg = wait(0, port)
            print "HomeScreenAPI.brs - [GetHomeScreenData] Message received, type: " + type(msg)
            
            if type(msg) = "roUrlEvent"
                code = msg.GetResponseCode()
                body = msg.GetString()
                print "HomeScreenAPI.brs - [GetHomeScreenData] HTTP Status Code: " + code.ToStr()
                print "HomeScreenAPI.brs - [GetHomeScreenData] Response body length: " + body.Len().ToStr()
                print "HomeScreenAPI.brs - [GetHomeScreenData] Raw response: " + body
                
                if code = 200 or code = 206
                    print "HomeScreenAPI.brs - [GetHomeScreenData] SUCCESS: Valid status code"
                    response = ParseJson(msg.GetString())
                    if response <> invalid
                        print "HomeScreenAPI.brs - [GetHomeScreenData] JSON parsed successfully"
                        print "HomeScreenAPI.brs - [GetHomeScreenData] Response data: " + FormatJSON(response)  
                        m.top.responseData = msg.GetString()
                        print "HomeScreenAPI.brs - [GetHomeScreenData] Response data set successfully"
                    else
