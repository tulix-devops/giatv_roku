sub init()
    ...
    m.videoPlayer.observeField("state", "stateChanged")
...
sub stateChanged()
    ...
    print m.videoPlayer.errorCode
end sub


sub onVideoPlayerWidthChanged()
    width = m.top.width

    if width <> invalid and width <> ""
        m.videoPlayer.width = width
    end if 

end sub

sub onVideoPlayerHeightChanged()
    height = m.top.height

    if height <> invalid and height <> ""
